...
export class BookController {
  ...
  async getPopularSearchTerms(@Query('limit') limit?: number): Promise<any> {
    return this.bookService.getPopularSearchTerms(limit || 10);
  }
  ...
  async getRecentSearchTerms(
    ...
    return this.bookService.getRecentSearchTerms(user.id, limit || 5);
  }

  /**
   * 검색어 저장 API
   */
  @Post('save-search-term')
  @IsPublic()
  async saveSearchTerm(
    ...
    await this.bookService.saveSearchTerm(term, userId, bookId);
    return { success: true };
  }
}
